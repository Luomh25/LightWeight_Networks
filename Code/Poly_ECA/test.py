...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for iteration, (images, targets) in enumerate(val_loader):
            ...
            tb_writer.add_scalar(tags[1], acc, epoch)
            tb_writer.add_scalar(tags[2], optimizer.param_groups[0]["lr"], epoch)            
            # 在验证循环中的适当位置执行上述代码，累积测试集的损失和正确预测的数量

        # 计算平均损失和准确率
        train_loss_avg = train_loss_total / train_total
        train_acc_avg = train_correct_total / train_total

        val_loss_avg = val_loss_total / val_total
        val_acc_avg = val_correct_total / val_total

        # 输出训练集和测试集的损失和准确率
        print("Train Loss: {:.4f} | Train Acc: {:.4f}".format(train_loss_avg, train_acc_avg))
        print("Val Loss: {:.4f} | Val Acc: {:.4f}".format(val_loss_avg, val_acc_avg))
        torch.save(model.state_dict(), "./weights/model-{}.pth".format(epoch))
            # 在训练循环中的适当位置执行上述代码，累积训练集的损失和正确预测的数量
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str,
                        default="/root/0424/vessel")

    # download model weights
    parser.add_argument('--weights', type=str,
                        default='/root/efficientnetb0.pth',
